 ...
 * 每个Nacos服务器节点负责完全负责一块数据。 每个数据块均由其负责的服务器生成，删除和同步。 所以每个Nacos服务器仅处理全部服务数据的一部分的写入。
 ...
 * 同时，每台Nacos服务器都会收到其他Nacos服务器的数据同步，因此每台Nacos服务器最终都会拥有完整的数据集。
...
public class DistroConsistencyServiceImpl implements EphemeralConsistencyService {
    ...
    public void put(String key, Record value) throws NacosException {
        onPut(key, value); // // 处理本地注册列表
        taskDispatcher.addTask(key); // 添加阻塞任务，同步信息到其他集群节点
    }
    ...
    public void onPut(String key, Record value) {
        ...
        if (KeyBuilder.matchEphemeralInstanceListKey(key)) {
            ...
            datum.value = (Instances) value; //当前服务的所有实例
            datum.key = key;
