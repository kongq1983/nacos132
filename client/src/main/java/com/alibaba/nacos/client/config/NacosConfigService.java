...
public class NacosConfigService implements ConfigService {
    ...
    private String getConfigInner(String tenant, String dataId, String group, long timeoutMs) throws NacosException {
        group = null2defaultGroup(group); // 默认:DEFAULT_GROUP
        ...
        // 优先使用本地配置
        String content = LocalConfigInfoProcessor.getFailover(agent.getName(), dataId, group, tenant); // 先从本地缓存文件
        if (content != null) {
        ...
        try { // ct[0]:内容  ct[1]:文本类型（text、xml、yaml）
