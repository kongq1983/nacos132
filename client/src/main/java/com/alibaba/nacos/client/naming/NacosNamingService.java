...
/** NacosNamingService是NamingService接口的实现类
 * Nacos Naming Service.
 * NacosNamingService还起到了初始化其他核心类的作用，因为对外提供的方法都是委托给其他核心类处理的。按顺序将依次初始化EventDispatcher、NamingProxy、BeatReactor、HostReactor
 * @author nkorange
 */
...
public class NacosNamingService implements NamingService {
    ...
    /** 维护本地订阅的服务注册表信息  HostReactor用于获取、保存、更新各Service实例信息。 */
    private HostReactor hostReactor;
    /** 发送心跳的 */
    private BeatReactor beatReactor;
    /** 事件分发器，就是你订阅了某个服务，然后服务改变的时候，nacos服务端就会通知到client端，client端收到这个通知后，就会将这个事件通知到观察者 */
    ...
    private void init(Properties properties) throws NacosException {
        ...
        this.hostReactor = new HostReactor(this.eventDispatcher, this.serverProxy, beatReactor, this.cacheDir,
                isLoadCacheAtStart(properties), initPollingThreadCount(properties)); // 维护本地订阅的服务注册表信息
    }
