...
public class CacheData {
    ...
    void checkListenerMd5() {
        for (ManagerListenerWrap wrap : listeners) {
            if (!md5.equals(wrap.lastCallMd5)) { // 判断md5是否变了
                safeNotifyListener(dataId, group, content, type, md5, wrap); // 这里会触发回调 listener.receiveConfigInfo(contentTmp)
            }
        }
    }
        ...
        Runnable job = new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    listener.receiveConfigInfo(contentTmp); //触发回调
